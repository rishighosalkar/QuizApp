...
namespace QuizApi.Controllers
{
    ...
    public class ParticipantController : Controller
    {
        ...
        //api/Participant/UpdateParticipantRole/5
        [HttpPut]
        [Route("UpdateParticipantRole/{id}")]
        public async Task<IActionResult> PutParticipantRole(int id, ParticipantRole _participantRole)
        {
            if (id != _participantRole.ParticipantId)
            {
                return BadRequest();
            }

            Participant updateParticipant = _context.Participants.Find(id);
            updateParticipant.Role = _participantRole.Role;
            _context.Entry(updateParticipant).State = EntityState.Modified;
            try
            {
                await _context.SaveChangesAsync();
            }
